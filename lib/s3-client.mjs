...
export default class {
  ...
  async uploadPart (buffer, key, partNumber, uploadId) {
    if (!uploadId && !(uploadId instanceof String)) {
      throw new Error('uploadId is not provided. Have you called createMultipartUpload before?')
    }

    if (!buffer) throw new Error('buffer is not provided')
    if (!key) throw new Error('key is not provided')
    if (!partNumber) throw new Error('part number is not provided')
